        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="btn-group" role="group" aria-label="manage">
                    <a href="{{ route('vendor.create') }}" class="btn btn-sm btn-primary">Create</a>
                </div>
                        ...
                        <tbody>
                            @foreach ($vendors as $vendor)
                                <tr>
                                    ...
                                    <td>{{ number_format($vendor->payable) }}</td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="manage">
                                            <a href="{{ route('vendor.edit', $vendor->id) }}" class="btn btn-secondary btn-sm">Edit</a>
                                            <button type="button" class="btn btn-danger btn-sm delete-btn" data-id="{{ $vendor->id }}" data-name="{{ $vendor->name }}">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(event) {
                    ...
                    const vendor_id = this.getAttribute('data-id');
                    const vendor_name = this.getAttribute('data-name');
                    Swal.fire({
                        title: 'Are you sure?',
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action',
                                `{{ route('vendor.destroy', ':id') }}`.replace(
                                    ':id', vendor_id));
                            ...
                            name.setAttribute('value', vendor_name);
