...
@section('content')
                    ...
                    <div class="card-body">
                        <form action="{{ route('journal.update', $journal->id) }}" method="POST" id="journal-form">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="id" name="id" value="{{ $journal->id }}" readonly>
                                </div>
                            </div>
                                        ...
                                        <tbody>
                                            @foreach($journal->journal_detail as $index => $detail)
                                                <tr>
                                                    <td>
                                                        <select name="details[{{ $index }}][account_id]" class="form-control select2" required>
                                                            ...
                                                            @foreach($accounts as $account)
                                                                <option value="{{ $account->id }}" {{ $account->id == $detail->account_id ? 'selected' : '' }}>
                                                                    {{ $account->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <input type="text" name="details[{{ $index }}][description]" class="form-control" value="{{ $detail->description }}" placeholder="Description" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" name="details[{{ $index }}][debit]" class="form-control debit" value="{{ $detail->debit }}" step="0.01" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" name="details[{{ $index }}][credit]" class="form-control credit" value="{{ $detail->credit }}" step="0.01" required>
                                                    </td>
                                                    <td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            let rowCount = {{ count($journal->journal_detail) }};
