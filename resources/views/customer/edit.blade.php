    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('customer.index') }}">{{ ucwords(str_replace('_', ' ', 'customer')) }}</a></li>
                    ...
                    <div class="card-body">
                        <form action="{{ route('customer.update', $customer->id) }}" method="POST">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control select2" id="business" name="business_id" width="100%" required autofocus>
                                        <option disabled {{ is_null($customer->business_id) ? 'selected' : '' }}>
                                            Select a {{ ucwords(str_replace('_', ' ', 'business')) }} :
                                        </option>
                                        @foreach($businesses as $business)
                                            <option value="{{ $business->id }}"
                                                {{ $customer->business_id == $business->id ? 'selected' : '' }}>
                                                {{ $business->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="phone_number" name="phone_number" value="{{ $customer->phone_number }}" required autofocus>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="address" required autofocus>{{ $customer->address }}</textarea>
                                </div>
                            </div>
