...
@section('title')
    {{ ucwords(str_replace('_', ' ', 'edit_supplier')) }}
@endsection
                    ...
                    <div class="card-body">
                        <form action="{{ route('supplier.update', $supplier->id) }}" method="POST">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control select2" id="business" name="business_id" width="100%" required autofocus>
                                        <option disabled {{ is_null($supplier->business_id) ? 'selected' : '' }}>
                                            Select a {{ ucwords(str_replace('_', ' ', 'business')) }} :
                                        </option>
                                        @foreach($businesses as $business)
                                            <option value="{{ $business->id }}"
                                                {{ $supplier->business_id == $business->id ? 'selected' : '' }}>
                                                {{ $business->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="phone_number" name="phone_number" value="{{ $supplier->phone_number }}" required autofocus>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="address" required autofocus>{{ $supplier->address }}</textarea>
                                </div>
                            </div>
